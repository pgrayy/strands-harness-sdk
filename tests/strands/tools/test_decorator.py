...
Args:
    ...
    result = list(test_tool.stream(tool_use))[-1]
...
def test_agent_parameter_passing():
    ...
    # Test with agent
    result = list(test_tool.stream(tool_use, agent=mock_agent))[-1]
...
def test_class_method_handling():
    ...
    result = list(instance.test_method.stream(tool_use))[-1]
...
def test_tool_as_adhoc_field():
    ...
    result2 = list(instance.field.stream({"toolUseId": "test-id", "input": {"param": "example"}}))[-1]
...
def test_default_parameter_handling():
    ...
    tool_use = {"toolUseId": "test-id", "input": {"required": "hello"}}
    result = list(tool_with_defaults.stream(tool_use))[-1]
...
def test_empty_tool_use_handling():
    ...
    result = list(test_tool.stream({}))[-1]
    assert result["status"] == "error"
    assert "unknown" in result["toolUseId"]

    # Test with missing input
    result = list(test_tool.stream({"toolUseId": "test-id"}))[-1]
    assert result["status"] == "error"
...
def test_traditional_function_call():
    ...
    result = list(add_numbers.stream(tool_use))[-1]
    assert result["status"] == "success"
...
def test_multiple_default_parameters():
    ...
    result = list(multi_default_tool.stream(tool_use))[-1]
    assert result["status"] == "success"
...
def test_return_type_validation():
    ...
    result = list(int_return_tool.stream(tool_use))[-1]
    assert result["status"] == "success"
    ...
    result = list(union_return_tool.stream(tool_use))[-1]
    assert result["status"] == "success"
...
def test_tool_with_no_parameters():
    ...
    # Test tool use call
    tool_use = {"toolUseId": "test-id", "input": {}}
    result = list(no_params_tool.stream(tool_use))[-1]
    assert result["status"] == "success"
...
def test_complex_parameter_types():
    ...
    result = list(complex_type_tool.stream(tool_use))[-1]
    assert result["status"] == "success"
...
def test_custom_tool_result_handling():
    ...
    result = list(custom_result_tool.stream(tool_use))[-1]
...
def test_detailed_validation_errors():
    ...
    result = list(validation_tool.stream(tool_use))[-1]
    assert result["status"] == "error"
...
def test_tool_complex_validation_edge_cases():
    ...
    tool_use = {"toolUseId": "test-id", "input": {"param": None}}
    result = list(edge_case_tool.stream(tool_use))[-1]
    assert result["status"] == "success"
    assert result["content"][0]["text"] == "None"
...
def test_tool_method_detection_errors():
    ...
    direct_result = list(instance.test_method.stream({"toolUseId": "test-id", "input": {"param": "direct"}}))[-1]
    ...
    tool_use_result = list(standalone_tool.stream({"toolUseId": "test-id", "input": {"p1": "value1"}}))[-1]
...
def test_tool_general_exception_handling():
    ...
    for error_type in error_types:
        ...
        result = list(failing_tool.stream(tool_use))[-1]
        assert result["status"] == "error"
...
def test_tool_with_complex_anyof_schema():
    ...
    result = list(complex_schema_tool.stream(tool_use))[-1]
    assert result["status"] == "success"
