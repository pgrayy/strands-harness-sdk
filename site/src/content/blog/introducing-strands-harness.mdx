...
Strands harness, built on the [Strands Harness SDK](https://github.com/strands-agents/harness-sdk), contains defaults for prompt caching and context management. Our [default context management](/docs/user-guide/harness/configure/context-and-caching/) largely drove the token-efficiency and accuracy: tool results over ~1500 tokens get truncated, summarization (compaction) triggers when the context window hits above 85%, and context recovery runs within the loop if there's an overflow.
...
You can easily install Strands harness with `pip install strands-harness` or `npm install @strands-agents/harness`.

For an interactive experience, download the Strands CLI `npm install @strands-agents/cli`.
